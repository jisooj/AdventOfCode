...
public class Solution {
  public static void main(String[] args) {
    ...
    while (input.hasNextLine()) {
      ...
      boolean isFullyOverlap = isOverlap(pair[0], pair[1]) || isOverlap(pair[1], pair[0]);
      if (isFullyOverlap) {
        count++;
      }
    }
  ...
  public static boolean isOverlap(String s1, String s2) {
    String[] range1 = s1.split("-");
    String[] range2 = s2.split("-");

    int[] rangeInt1 = new int[] { Integer.parseInt(range1[0]), Integer.parseInt(range1[1]) };
    int[] rangeInt2 = new int[] { Integer.parseInt(range2[0]), Integer.parseInt(range2[1]) };
    return (rangeInt1[0] <= rangeInt2[0] && rangeInt2[0] <= rangeInt1[1]) && // Lower bound check
      (rangeInt1[0] <= rangeInt2[1] && rangeInt2[1] <= rangeInt1[1]); // Upper bound check
  }
}
